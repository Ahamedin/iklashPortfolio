...
async function handler(req: NextRequest) {
  try {
    ...
    const systemPrompt = `You are an AI assistant helping a visitor draft an email to Iklash Ahamed. 
    The email should be written from the perspective of the visitor/user, addressed to Iklash.
    Use the user's prompt to determine the intent (e.g., job offer, collaboration, question).
    
    Requirements:
    - Address the email to "Iklash" or "Mr. Ahamed".
    ...
    const response = await fetch(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        ...
        headers: {
          ...
          "HTTP-Referer": "https://iklash-ahamed.vercel.app",
          "X-Title": "Iklash Ahamed Portfolio",
